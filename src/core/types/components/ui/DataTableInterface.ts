import type { ArrayOfObjects, Flatten, ObjectOfIC } from '@/core/types/GlobalTypes';
 ...
 */
export type PropsIC = {
  rows: RowsDataTableIC;
  columns: ColumnDataTableIC<any>[];
...
/**
 * Tipado para los rows de la tabla
 */
export type RowsDataTableIC = QTableProps['rows'];

/**
 * Tipado para las columnas de la tabla
 */
export type ColumnDataTableIC<T extends ArrayOfObjects> = {
  /**
   * Indicates if column can be exported
   */
  dontExport?: boolean;
   ...
   */
  field: (T extends any[] ? keyof Flatten<ObjectOfIC<T>> : string) | null;
  /**
