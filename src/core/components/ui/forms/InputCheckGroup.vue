...
import type { CheckGroupIC } from '@/core/types/components/ui/FormsInterface';
...
const props = withDefaults(defineProps<CheckGroupIC>(), {
  label: 'Radio Group',
  required: false,
...
const emits = defineEmits<{
  (e: 'update:modelValue', value: CheckGroupIC['modelValue']): void;
  (e: 'update:errorMessages', value: CheckGroupIC['errorMessages']): void;
}>();
...
const model = computed({
  get() {
    if (Array.isArray(props.modelValue) && typeof props.modelValue[0] === 'number') {
      return props.modelValue.map((value) => value.toString());
    }
